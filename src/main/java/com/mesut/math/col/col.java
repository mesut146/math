...
public class col {
    ...
    public static String col(Linear linear) {
        if (linear.a % 2 == 0) {
            Linear res;
            if (linear.b % 2 == 0) {
                res = linear.div(2);
                ...
                res = linear.mul(3).add(1);
            }
            return (res.isOdd() ? "(" + res + ")" : res) + " " + col(res);
        }
        return "";
    }
    ...
    static void b() {
        for (int k = 4; k < 50; k += 4) {
            for (int i = 1; i < k; i += 2) {
                Linear linear = new Linear(k, i);
                System.out.println(linear + " " + col(linear));
            }
            System.out.println("------------");
        }
    }
